...
char *_whch(char *cmd, char **_environment)
{
	...
	if (path)
	{
		str_path = _strngduplicate(path);
		len1_changecmd = _strnglength(cmd);
		tok_path = _strngtoken(str_path, ":");
		j = 0;
		while (tok_path != NULL)
		{
			if (is_cdirectory(path, &j))
				if (stat(cmd, &st) == 0)
					return (cmd);
			len1_directory = _strnglength(tok_path);
			str_dir = malloc(len1_directory + len1_changecmd + 2);
			_strngcopy(str_dir, tok_path);
			_strngcat(str_dir, "/");
			_strngcat(str_dir, cmd);
			_strngcat(str_dir, "\0");
...
int chck_err_cmd(char *dir, data_shell *datash)
{
	...
	if (_strngcompare(datash->arguments[0], dir) != 0)
	{
...
int cmd_execute(data_shell *datash)
{
	...
	if (executor == 0)
	{
		str_dir = _whch(datash->arguments[0], datash->_evirnment);
	...
	if (str_pd == 0)
	{
		if (executor == 0)
			...
			str_dir = datash->arguments[0];
		execve(str_dir + executor, datash->arguments, datash->_evirnment);
	}
	else if (str_pd < 0)
	{
		perror(datash->av[0]);
